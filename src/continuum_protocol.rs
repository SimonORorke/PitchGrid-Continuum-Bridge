...
impl ContinuumProtocol {
    ...
    fn on_init_data_download_completed(&self) {
        trace!("on_init_data_download_completed: Stopping download monitor");
...
impl MidiInputListener for ContinuumProtocol {
    fn on_message(&self, message: &[u8]) {
        ...
        if self.is_monitoring_download.load(Ordering::Relaxed) {
            ...
            {
                trace!("on_message: Starting download monitor");
        ...
        if let LiveEvent::Midi { channel, message } = event {
            match message {
                MidiMessage::Controller { controller, value } => {
                    ...
                    if controller == 109 {
                        if value == 40 {
                            trace!("on_message_received: EndSysNames");
                            *self.download_status.lock().unwrap() = DownloadStatus::EndSysNames;
                            return;
                        }
                        if value == 49 {
                            trace!("on_message_received: BeginSysNames");
                        ...
                        if value == 54 {
                            trace!("on_message_received: BeginUserNames");
                        ...
                        if value == 55 {
                            trace!("on_message_received: EndUserNames");
                ...
                MidiMessage::ProgramChange { program } => {
                    ...
                    if channel1 == 16 {
                        ...
                        trace!("on_message_received: ProgramChange ch16 program {}", u8::from(program));
                        ...
                        if download_status == DownloadStatus::EndUserNames
                            ...
                            trace!("on_message_received: End of download");
                            self.on_init_data_download_completed();
                            return;
                        }
                        if download_status == DownloadStatus::Complete {
                            ...
                            // from disc.
                            trace!("on_message_received: Program, preset selected");
                            if let Some(listener) = self.listener() {
