...
fn main() {
    ...
    // TODO: Does MAIN_WINDOW_WEAK initialisation need to be in a separate block?
    {
...
fn init_ui_handlers(main_window: &MainWindow, controller: SharedController) {
    let window_weak = main_window.as_weak();
    {
        ...
        main_window.window().on_close_requested(move || {
            handle_close_request(window_weak.clone(), &controller)
        });
    }
    {
        let controller: SharedController = Arc::clone(&controller);
...
fn handle_close_request(
    main_window_weak: Weak<MainWindow>, controller: &SharedController) -> CloseRequestResponse {
    ...
    if let Err(err) = controller.lock().unwrap().close() {
...
//             if can_update_tuning {
//                 show_pitchgrid_status(
//                     main_window, "Updating instrument tuning", SlintMessageType::Info);
//             } else {
//                 show_pitchgrid_status(
//                     main_window,
//                     "Cannot updating tuning. Connect instrument input/output.",
//                     SlintMessageType::Error);
//             }
//         });
//     }
// }
...
//                                   "Instrument tuning updated", SlintMessageType::Info);
//         });
//     }
// }
...
fn get_selected_port_index(port_strategy: &dyn PortStrategy) -> usize {
    if let Some(main_window_weak) = main_window_weak_static_clone() {
        with_main_window(main_window_weak.clone(), move |main_window| {
            main_window.get_selected_port_index()
        });
    }
}
...
fn set_selected_port_index(index: usize, port_strategy: &dyn PortStrategy) {
    if let Some(main_window_weak) = main_window_weak_static_clone() {
        with_main_window(main_window_weak.clone(), move |main_window| {
            main_window.set_selected_port_index(index);
        });
    }
}
...
//
// fn set_ports_model(
//         main_window: &MainWindow, controller: &Controller, port_strategy: &dyn PortStrategy) {
//     let port_items: Vec<ComboBoxItem> =
//         controller.port_names(port_strategy)
//         .iter()
//         .map(|text| ComboBoxItem { text: text.into() })
...
//     show_message(main_window, message, SlintMessageType::Error);
// }

// fn show_info(main_window: &MainWindow, message: impl Into<SharedString>) {
//     show_message(main_window, message, SlintMessageType::Info);
// }

// fn show_message(main_window: &MainWindow, message: impl Into<SharedString>, message_type: SlintMessageType) {
...
//     show_pitchgrid_status(
//         main_window, "Pitchgrid OSC is connected", SlintMessageType::Info);
// }
...
//     main_window: &MainWindow, message: impl Into<SharedString>, message_type: SlintMessageType) {
...
//     show_message(main_window, message, SlintMessageType::Warning);
// }
