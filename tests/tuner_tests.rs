...
fn on_tuning_received() {
    ...
    // Rounding Initial, Rounding Override, Rounding Rate, active pitch table
    assert_that!(mock_midi_sender().control_change_count, eq(774));
    // Active pitch table
...
fn send_current_preset_update() {
    ...
    // instrument).
    assert_that!(
        cumulative_sent_control_change_count, eq(first_time_sent_control_change_count + 4));
    // Rounding Mode Normal, because Rounding Rate is on
...
fn create_tuner() -> Tuner {
    ...
    let tuner = Tuner::new(mock_continuum_protocol.clone(), mock_midi_sender.clone());
